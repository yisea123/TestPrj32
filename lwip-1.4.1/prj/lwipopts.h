...
#define MEM_SIZE                4800 	//内存堆heap大小
...
#define MEMP_NUM_TCP_SEG        50//15		//MEMP_NUM_TCP_SEG:最多同时在队列中的TCP段数量
...
//pbuf选项
#define PBUF_POOL_SIZE          7		//PBUF_POOL_SIZE:pbuf内存池个数
...
#define TCP_SND_BUF             (4*TCP_MSS)//(4*TCP_MSS)		//TCP发送缓冲区大小(bytes).
#define TCP_SND_QUEUELEN        (2* TCP_SND_BUF/TCP_MSS)//(2* TCP_SND_BUF/TCP_MSS)	//TCP_SND_QUEUELEN: TCP发送缓冲区大小(pbuf).这个值最小为(2 * TCP_SND_BUF/TCP_MSS)
#define TCP_WND                 (2*TCP_MSS)//(2*TCP_MSS)		//TCP发送窗口
