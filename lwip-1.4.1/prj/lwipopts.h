...
#define MEM_SIZE                10000//4800 	//内存堆heap大小
...
#define TCP_SND_BUF             (10/*4*/*TCP_MSS)//(4*TCP_MSS)		//TCP发送缓冲区大小(bytes).
#define TCP_SND_QUEUELEN        (10/*2*/* TCP_SND_BUF/TCP_MSS)//(2* TCP_SND_BUF/TCP_MSS)	//TCP_SND_QUEUELEN: TCP发送缓冲区大小(pbuf).这个值最小为(2 * TCP_SND_BUF/TCP_MSS)
#define TCP_WND                 (6/*2*/*TCP_MSS)//(2*TCP_MSS)		//TCP发送窗口
