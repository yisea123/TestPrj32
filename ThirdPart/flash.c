...
{  
	u8 byte=0;   
	FLASH_EN;                            //使能器件   
	FLASH_ReadWriteByte(W25X_ReadStatusReg);    //发送读取状态寄存器命令    
...
{   
	FLASH_EN;                            //使能器件   
	FLASH_ReadWriteByte(W25X_WriteStatusReg);   //发送写取状态寄存器命令    
...
u16 SPI_Flash_ReadID(void)
{
	u16 Temp = 0;	  
	FLASH_EN;				    
	FLASH_ReadWriteByte(0x90);//发送读取ID命令	    
...
void SPI_Flash_Write_Page(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
  ...
  FLASH_ReadWriteByte((u8)((WriteAddr)>>16)); //发送24bit地址    
  FLASH_ReadWriteByte((u8)((WriteAddr)>>8));   
  FLASH_ReadWriteByte((u8)WriteAddr);   
  for(i=0;i<NumByteToWrite;i++)FLASH_ReadWriteByte(pBuffer[i]);//循环写数  
	FLASH_DS;                            //取消片选 
...
{   
    ...
    SPI_Flash_Wait_Busy();   
  	FLASH_EN;                            //使能器件   
    FLASH_ReadWriteByte(W25X_SectorErase);      //发送扇区擦除指令 
    FLASH_ReadWriteByte((u8)((Dst_Addr)>>16));  //发送24bit地址    
...
{ 
  	FLASH_EN;                            //使能器件   
    FLASH_ReadWriteByte(W25X_PowerDown);        //发送掉电命令  
	  FLASH_DS;                            //取消片选     	      
    Delay(6);                               //等待TPD  
}   
//唤醒
