...
void ReadCoil(CDV_INT08U* rxBuf,CDV_INT08U rxLen){//获取位状态
	...
	else if(addr > 0xAAAA || addr + num > 0xAAAA)
	{
...
//	ReadRegisterCmdParse(addr,num);
	...
	#endif
	txbuf[1]=0x03;
	memcpy(&txbuf[3], &USART_TX_BUF[3] , len-5);
	AddTx(txbuf, len-2);
	USART2_Send(USART_TX_BUF,len);
...
//	}
	for( i = 0; i< num ; i++) {
		...
		WriteMultiRegisterCmdParse(addr+i);
	}
...
void WriteMultiRegisterCmdParse(CDV_INT16U addr)
{
	switch(addr) {
		case REG_DA1:
			Dac1_Set_Vol(MODBUS_REG_ADDR(REG_DA1)*384/1000);
		 break;
		case REG_DA2:
			Dac2_Set_Vol(MODBUS_REG_ADDR(REG_DA2)*384/1000);
		 break;
		case REG_DA3:
			WriteToDAC1_7512(MODBUS_REG_ADDR(REG_DA3)*385/1000);
		 break;
		case REG_DA4:
			WriteToDAC2_7512(MODBUS_REG_ADDR(REG_DA4)*385/1000);
		 break;
		case REG_DA5:
			WriteToDAC3_7512(MODBUS_REG_ADDR(REG_DA5)*385/1000);		
		 break;
		case REG_DA6:
			WriteToDAC4_7512(MODBUS_REG_ADDR(REG_DA6)*385/1000);
		 break;
    default:
...
#include <stm32f10x.h>
		...
		if(USART_GetFlagStatus(USART3,USART_FLAG_TC)==SET)	//发送完成 进入接收模式
		{
			RS485_TX_EN=0;
